...
import { T, KB_COLORS, THEMES } from '@/lib/theme'
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 20px)', gap: 6 }}>
        {KB_COLORS.map((hex, i) => {
          const isActive = hex === null ? !currentColor : hex === currentColor
          if (hex === null) {
            // First swatch: "no colour" / clear
            return (
              <button
                key="clear"
                title="No colour"
...
export default function Column({ column, onDeleteColumn, onRenameColumn, onSelectCard, onAddCard, swimlanes }: Props) {
  ...
  // headerColor is always a raw hex string (e.g. '#ef4444') or null
  const activeHex = column.headerColor && column.headerColor.startsWith('#')
    ? column.headerColor
    : undefined
