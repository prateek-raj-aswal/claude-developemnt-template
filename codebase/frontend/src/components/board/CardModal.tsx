...
export default function CardModal({ card, columnName, boardId, onClose, onUpdate, onDelete }: Props) {
  ...
  async function handleStartDateChange(value: string) {
    ...
    // Validate: if both dates set and startDate > dueDate, show error and abort
    if (value && dueDate && value > dueDate) {
      setStartDateError('Start date must be before or equal to due date')
      return
    }

    try {
      await api.patch(`/api/v1/cards/${card.id}`, { startDate: value || null })
    } catch {
      setStartDate(prevStartDate) // rollback on PATCH error
      setStartDateError('Failed to save start date')
    }
  }
        ...
        <div style={{ display: 'flex', minHeight: 0, flex: 1, overflow: 'hidden', flexDirection: isMobile ? 'column-reverse' : 'row' }}>
          ...
          <div style={isMobile ? {
            ...
            <PropRow label="Start date">
              <label htmlFor="card-start-date" style={{ display: 'none' }}>Start date</label>
              <input
                id="card-start-date"
                ...
                style={{
                  ...
                  color: T.text,
                  background: T.card,
                  border: `1px solid ${startDateError ? T.danger : T.cardBorder}`, borderRadius: 4,
