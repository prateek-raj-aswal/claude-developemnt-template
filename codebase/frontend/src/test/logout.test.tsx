...
// BottomNav now renders SettingsModal (US-1821); stub it to avoid deep render
vi.mock('@/components/ui/SettingsModal', () => ({
  default: ({ open, showSignOut }: { open: boolean; showSignOut?: boolean }) =>
    open ? <div data-testid="settings-modal">{showSignOut && <button>Sign out</button>}</div> : null,
}))
...
describe('TC-003: BottomNav renders a logout affordance on mobile', () => {
  it('has a settings button that exposes sign-out when isMobile is true', () => {
    // US-1821: standalone "Log out" button replaced with Settings tab that opens
    // SettingsModal (with showSignOut=true). The affordance is the Settings button.
    render(<BottomNav />)
    const settingsBtn = screen.getByRole('button', { name: /settings/i })
    expect(settingsBtn).toBeInTheDocument()
    // Clicking it should reveal the settings modal (with sign-out inside)
