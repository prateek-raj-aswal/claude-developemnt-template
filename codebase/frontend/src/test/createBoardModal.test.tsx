...
describe('TC-2: Description textarea is present in the form', () => {
  it('renders a textarea for description', () => {
    ...
    // Placeholder updated in US-1817 to "What is this board for? (optional)"
    const textarea = screen.getByPlaceholderText(/what is this board for/i)
...
describe('TC-3: Submit calls onCreate with name and description', () => {
  it('passes both name and description to onCreate', async () => {
    ...
    // US-1817: onCreate now receives (name, description|null, emoji)
    expect(onCreate).toHaveBeenCalledWith('My Board', 'A great board', expect.any(String))
  })
})
...
describe('TC-4: Submit without description calls onCreate with name only', () => {
  it('passes name and undefined when description is empty', async () => {
    ...
    // US-1817: empty description becomes null; emoji defaults to '◇'
    expect(onCreate).toHaveBeenCalledWith('Quick Board', null, '◇')
  })
})
